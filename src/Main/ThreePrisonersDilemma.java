...
//			return new DonaldDuck();
	...
	boolean verbose = true; // set verbose = false if you get too much text
							// output
	...
	void runTournament() {
		...
		// Note that we include duplicates: two copies of your strategy will
		// play once
		// against each other strategy, and three copies of your strategy will
		// play once.
		...
		for (int i = 0; i < numPlayers; i++)
			for (int j = i; j < numPlayers; j++)
				for (int k = j; k < numPlayers; k++) {
					...
					Player A = makePlayer(i); // Create a fresh copy of each
												// player
					Player B = makePlayer(j);
					Player C = makePlayer(k);
					int rounds = 90 + (int) Math.rint(20 * Math.random()); // Between
																			// 90
																			// and
																			// 110
																			// rounds
					float[] matchResults = scoresOfMatch(A, B, C, rounds); // Run
																			// match
